...
## @knitr msit-fnames
# CCB
ccb_info <- subset(subinfo, study=="CCB")
ccb_info$subject <- factor(ccb_info$subject)
ccb_fnames_df <- ddply(ccb_info, .(subject, scan, run), function(sdf) {
    find_path <- file.path(
                    basedir, "CCB/analysis", 
                    paste(sdf$subject, "_scan", sdf$scan, "_MSIT", sdf$run, "_*_RSN_TCs.1D", sep="")
                )
...
# CCD
ccd_info <- subset(subinfo, study=="CCD")
ccd_info$subject <- factor(ccd_info$subject)
ccd_fnames_df <- ddply(ccd_info, .(subject, scan, run), function(sdf) {
    find_path <- file.path(
                    basedir, "analysis/subjects", sdf$subject, 
                    "msit", sprintf("run_%02i", sdf$run), 
                    "rsn10.1D"
                )
    paths <- Sys.glob(find_path)
    if (length(paths) != 1) {
...
ccb_event_tpts <- data.frame(
    xmin = c(0, which(tmp!=0) + 1 + 3), # +1 to adjust for relative; +3 to adjust for HRF delay
...
# CCB
ccb_msit_tcs <- daply(ccb_fnames_df, .(subject), function(sdf) {
    fnames <- sdf$infiles
    tcs <- laply(fnames, function(x) as.matrix(read.table(x))[,dmn], .drop=FALSE)
    tc <- colMeans(tcs)
    tc
})
ccb_msit_tc_ave <- data.frame(
    timepoint = 1:ncol(ccb_msit_tcs), 
    bold = colMeans(ccb_msit_tcs)
)
# CCD
ccd_msit_tcs <- daply(ccd_fnames_df, .(subject), function(sdf) {
    fnames <- sdf$infiles
    tcs <- laply(fnames, function(x) as.matrix(read.table(x))[,dmn], .drop=FALSE)
    tc <- colMeans(tcs)
    tc
...
p <- ggplot(ccb_msit_tc_ave) +
        ...
        scale_fill_manual(name="Block", breaks=c("Fixation", "Coherent", "Incoherent"), 
                            values=brewer.pal(4, "Pastel2")[-4]) + 
        geom_line(aes(x=timepoint, y=bold), color="darkblue", size=0.75) + 
        scale_x_discrete(name="Timepoint", limits=c(0,224), expand=c(0,0)) + 
...
# CCB
ccb_msit_cors <- ddply(ccb_fnames_df, .(subject), function(sdf) {
    fnames <- sdf$infiles
    tcs <- laply(fnames, function(x) as.matrix(read.table(x))[,dmn], .drop=FALSE)
    cors <- aaply(tcs, 1, cor, ccb_waver)
    z <- atanh(cors)
    sdf$inc.gt.con <- z
    sdf[,-4]
})
ggplot(ddply(ccb_msit_cors, .(subject), numcolwise(mean)), aes(x=inc.gt.con, fill=..count..)) + 
    ...
    ggtitle("CCB Dataset")
# CCD
ccd_msit_cors <- ddply(ccd_fnames_df, .(subject), function(sdf) {
    fnames <- sdf$infiles
    tcs <- laply(fnames, function(x) as.matrix(read.table(x))[,dmn], .drop=FALSE)
    cors <- aaply(tcs, 1, cor, ccd_waver)
    z <- atanh(cors)
    sdf$inc.gt.con <- z
    sdf[,-4]
})
ggplot(ddply(ccd_msit_cors, .(subject), numcolwise(mean)), aes(x=inc.gt.con, fill=..count..)) + 
...
df <- merge(ddply(ccd_msit_cors, .(subject), numcolwise(mean)), phenos)
names(df)[[2]] <- "correlation.dmn_with_task"
