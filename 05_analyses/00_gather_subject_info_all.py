...
ccd_all0 = DataFrame({'subject': subjects, 'condition': conditions, 'run': runs})
subjects = np.array([ ['CCD%03i' % i] for i in xrange(14, 28) if i not in [21,26]]).flatten()
runs = np.array([ 1 for i in xrange(len(subjects)) ])
conditions = np.array([ "REST" for i in xrange(len(subjects)) ])
ccd_all1 = DataFrame({'subject': subjects, 'condition': conditions, 'run': runs})
ccd_all = concat([ccd_all0, ccd_all1])
ccd_all['scan'] = 1
ccd_all['study'] = "CCD"
...
filt.to_csv(path.join(basepath, "scripts/data/ccb+ccd_filtered_all.csv"), index=False)
